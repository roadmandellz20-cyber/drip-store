          ...
          <div className="admin-card-list">
            {filtered.map((product) => {
              ...
              return (
                <div
                  key={product.slug}
                  className={`admin-card${!product.is_active ? " admin-card--faded" : ""}`}
                >
                  <div className="admin-card__top">
                    <span className="admin-card__sku">{product.slug}</span>
                    <div className="admin-card__badges">
                      <span className={`admin-badge ${statusBadge.cls}`}>{statusBadge.label}</span>
                      {product.is_limited && (
                        <span className="admin-badge admin-badge--yes">LTD</span>
                      )}
                      {product.is_new && (
                  ...
                  <div className="admin-card__name">{product.title}</div>
                  <div className="admin-card__meta">
                    <span><strong>{formatAmount(product.price_cents, product.currency)}</strong></span>
                    <span>STOCK <strong>{product.stock_qty ?? "—"}</strong></span>
                    <span>SOLD <strong>{product.sold_qty}</strong></span>
                  </div>
                  ...
                  {row?.error ? (
                    <div className="admin-card__error">{row.error}</div>
                  ) : null}
                </div>
              );
            })}
